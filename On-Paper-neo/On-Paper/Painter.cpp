...
void on_paper::Painter::draw_finger_tips(std::vector<Point> fingers, int size, cv::Scalar c) {
    if(c == Scalar(0,0,0))
        c=Scalar(255,255,255);
    for(const auto& finger_tip : fingers) {
        Point pp = finger_tip;
        transform_point(pp);
        circle(temp_canvas, pp, size/2, c, 4);
        circle(temp_canvas, pp, size*3, c, 12);
    }
}
